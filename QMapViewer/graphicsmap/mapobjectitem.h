...
class MapTableItem;
class MapScutcheonItem;

/**
 * @brief 地图对象
 * @details 可用于将图标添加到地图上
 * @note 1.该对象不受地图缩放影响，将会始终保持屏幕大小
 * 2.默认会阻断鼠标事件穿透到地图上，但是要想穿透鼠标事件请调用setAllowMouseEvent(false)
 * 3.pressed信号总是可以收到
 */
...
public:
    ...
    /**
     * @brief 设置图标，无效资源将使用默认图标
     */
    void setIcon(const QPixmap &pixmap);
    /**
     * @brief 设置图标为纯色，传QColor()可以取消纯色
     * @note 高分辨图片对该接口性能影响较大
    ...
    /**
     * @brief 设置是否允许鼠标事件，影响是否能够像QAbstractButton一样触发点击信号(但是可以收到press信号)以及切换选中状态
     */
    ...
    /**
     * @brief 节点速度
     * @note 支持航路节点速度存储 亦可指代当前节点初始速度
     */
    void setSpeed(double speed);
    double getSpeed() const;

    //MapTableItem * getMapTabel() const { return m_Suct; }
    MapScutcheonItem * getMapTabel() const { return m_Suct; }
public:
    /// 获取所有的实例
...
private:
    ...
    MapScutcheonItem *m_Suct = nullptr;
};
