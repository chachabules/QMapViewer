...
bool MapRangeLineOperator::mouseMoveEvent(QMouseEvent *event)
{
    ...
    double amu = m_line->endings().first.azimuthTo(m_line->endings().second);
    if (dis > 1000.0) {
        dis = dis * 1E-3;
        QString str = u8"距离：";
        str += QString::number(dis, 'f', 2) + QString("km");
        str += u8"方向：";
        str += QString::number(amu, 'f', 2) + QString("°");
        m_line->setText(str);
    }
    else if (dis < 1000.0) {
        dis = dis * 1E-3;
        QString str = u8"距离：";
        str += QString::number(dis,'f',2) + QString("m");
        str += u8"方向：";
        str += QString::number(amu, 'f', 2) + QString("°");
        m_line->setText(str);
        m_line->setText(str);
