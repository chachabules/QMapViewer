#ifndef MAPSCUTCHEONITEM_H
#define MAPSCUTCHEONITEM_H
...
class MapScutcheonItem : public QObject, public QGraphicsItem
{
    /*line of Scutcheon.*/
...
public:
    /**
     * @brief 继承父对象将同步一切事件，但旋转会导致子对象一起旋转。
     */
    MapScutcheonItem(MapObjectItem * parent = nullptr);
    ~MapScutcheonItem();
    /**
     * @brief 设置鼠标可拖拽
     */
    ...
    /**
     * @brief 设置连线图表的角度和长度  params: 这里长度为屏幕坐标,由于图表为ItemIgnoresTransformations 所以只需计算屏幕的终点位置
     */
    ...
    /**
     * @brief 设置启用固定方向. 添加作为子项时,会跟随父项转动；此时若想固定方向，启用固定方向，并调用SetHeading将angle设置为父转动角度负值.
     * @param 标识是否启用固定方向
     * @param 固定角度值
     */
