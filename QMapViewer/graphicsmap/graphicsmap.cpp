    ...
    m_rotation(0)
{
    ...
    qRegisterMetaType<TileSpec>("TileSpec");
    qRegisterMetaType<TileRegion>("TileRegion");
...
void GraphicsMapThread::requestTile(const TileRegion &region)
{
    // hide all tile items if tile resource path is invalid
    if(m_path.isEmpty()) {
        foreach(auto &tile , m_tileShowedSet) {
            hideItem(tile);
        }
    ...
    QSet<TileSpec> needToHideTileSet = m_tileTriedToShowdSet;
    m_tileTriedToShowdSet.clear();
    {
        QSetIterator<TileSpec> iter(curViewSet);
        while (iter.hasNext()) {
            auto tileSpec = iter.next();
            createAscendingTileCache(tileSpec, m_tileTriedToShowdSet);
        }
    }
    QSet<TileSpec> realToHideTileSet = needToHideTileSet - m_tileTriedToShowdSet;

    // update the scene tiles
    {
        QSetIterator<TileSpec> iter(m_tileTriedToShowdSet);
        while (iter.hasNext()) {
            auto &tileSpec = iter.next();
            showItem(tileSpec);
        }
    ...
    {
        QSetIterator<TileSpec> iter(realToHideTileSet);
        while (iter.hasNext()) {
            auto &tileSpec = iter.next();
            hideItem(tileSpec);
        }
...
void GraphicsMapThread::showItem(const TileSpec &tileSpec)
{
...
void GraphicsMapThread::hideItem(const TileSpec &tileSpec)
{
    // 看不见的直接不管
    if(!m_tileShowedSet.contains(tileSpec))
        return;
 ...
 */
QGraphicsPixmapItem *GraphicsMapThread::loadTileItem(const TileSpec &tileSpec)
{
...
void GraphicsMapThread::createAscendingTileCache(const TileSpec &tileSpec, QSet<TileSpec> &sets)
{
